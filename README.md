...
# Research UI: review scraped rows and fetch 10 Tavily images per topic
...
4. **Research UI** — `--serve` opens a local page; each row has **Find images** (up to 15 per row)
5. **GPT** — write debrief (use `--use-cache` to skip steps 1–3)

Output is written to `output/YYYY-MM-DD/debrief.html` and `debrief.json`.

Open the HTML in a browser and use **Print → Save as PDF** for a clean printable copy.

## Deploy (Vercel)

The Research UI and API routes deploy as a Python serverless function (`api/index.py`).
...
On Vercel, cache/output live under `/tmp` for the lifetime of a function instance. Use **Scrape today** on first visit (or after a cold start). Hobby plans cap function duration at 10s — scraping may require Pro (this repo sets `maxDuration: 300`).
